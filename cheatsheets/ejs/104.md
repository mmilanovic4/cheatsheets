...
- **Pro tip:** almost all JavaScript values have properties, the exceptions are `null` and `undefined`
- The two main ways to access properties in JavaScript are with a dot (`value.x`) and with square brackets (`value[x]`)
...
- JavaScript does this for bindings you declared with `let` or `const` - but not for standard bindings nor for bindings declared with `var` or `function`
- Back to the `Math` object - the number π (pi) - or at least the closest approximation that fits in a JavaScript number - is available as `Math.PI`
